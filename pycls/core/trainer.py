...
def setup_model():
    ...
    if cfg.FSDP.ENABLED:
        fsdp_config = {}
        fsdp_config["mixed_precision"] = cfg.FSDP.MIXED_PRECISION
        if cfg.FSDP.MIXED_PRECISION:
            fsdp_config["clear_autocast_cache"] = True
        fsdp_config["reshard_after_forward"] = not cfg.FSDP.DO_NOT_RESHARD_AFTER_FW
        if cfg.FSDP.LAYER_NORM_FP32:
            def do_wrap(m):
                m = auto_wrap_ln(m)
                return FSDP(m, **fsdp_config)

            model = do_wrap(model)
            ema = do_wrap(ema_model)
        else:
            with enable_wrap(wrapper_cls=FSDP, **fsdp_config):
                model = FSDP(model, **fsdp_config)
                ema = FSDP(ema_model, **fsdp_config)
    ...
    ema = model.cuda(device=cur_device)
...
def train_model():
    ...
    if cfg.FSDP.ENABLED:
        scaler = ShardedGradScaler(enabled=cfg.FSDP.MIXED_PRECISION)
    else:
